...
const Contact = () => {
    ...
    const [userContact, setUserContact] = useState({
      ...
      const handleSubmitContact = (e) => { // Added 'e' parameter to the function
        ...
        if (!userContact.username || !userContact.email || !userContact.message) {
         ...
         addDoc(collection(db ,'ContactUser'),{
          ...
          alert(`ThankYou for Joining With Us ${userContact.username}`)
          setUserContact({
            username: "",
            email: "",
            message: ""
    ...
    <div className="lg:w-2/3 md:w-1/2 bg-gray-300 rounded-lg overflow-hidden sm:mr-10 p-10 flex items-end justify-start relative">
      ...
      <div className="bg-white relative flex flex-wrap py-6 rounded shadow-md">
        ...
        <div className="lg:w-1/2 px-6 mt-4 lg:mt-0">
          ...
          <a className="text-indigo-500 leading-relaxed">frendsshop@email.com</a>
          <h2 className="title-font font-semibold text-gray-900 tracking-widest text-xs mt-4">PHONE</h2>
          <p className="leading-relaxed">883-534-3425</p>
        </div>
      </div>
    </div>
    <div className="lg:w-1/3 md:w-1/2 bg-white flex flex-col md:ml-auto w-full md:py-8 mt-8 md:mt-0">
      <h2 className="text-gray-900 text-lg mb-1 font-medium title-font">Contact Us</h2>
      <p className="leading-relaxed mb-5 text-gray-600">Post-ironic portland shabby chic echo park, Frends fashion axe</p>
      <div className="relative mb-4">
        ...
        onChange={handleContact} value={userContact.username} />
      </div>
      <div class="relative mb-4">
        <label for="email" className="leading-7 text-sm text-gray-600">Email</label>
        <input type="email" id="email" name="email" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out"
        onChange={handleContact} value={userContact.email}/>
      </div>
      <div class="relative mb-4">
        <label for="message" className="leading-7 text-sm text-gray-600">Message</label>
        <textarea type='text' id="message" name="message" className="w-full bg-white rounded border border-gray-300 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 h-32 text-base outline-none text-gray-700 py-1 px-3 resize-none leading-6 transition-colors duration-200 ease-in-out"
        value={userContact.message}  onChange={handleContact} ></textarea>
      </div>
